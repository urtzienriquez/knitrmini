...
call_block <- function(block) {
  ...
  if (!is.null(params$child)) {
    if (!is_blank(params[["code"]]) && getOption("knitr.child.warning", TRUE)) {
      warning(
...
eng_r <- function(options) {
  ...
  if (isTRUE(options$cache)) {
    if (isTRUE(options$cache.rebuild)) purge_cache(options)
    options$hash <- cache_name(options)
    cached <- block_cache_load(options)
    if (!is.null(cached)) {
      output <- paste(c(cached), collapse = "")
      if (options$include) {
        return(output)
      } else {
        return("")
  ...
  res <- if (is_blank(code)) {
    list()
  } else if (isFALSE(options$eval)) {
    as.source(code)
  } else {
    in_input_dir(
      evaluate(
        code,
        envir = env, new_device = FALSE,
  ...
  if (isTRUE(options$cache)) {
    block_cache_save(options, output, ls(envir = env, all.names = TRUE))
  }
...
sew <- function(res, options, inline = FALSE) {
  if (inline) {
    return(knit_hooks$get("inline")(res))
  }
  res <- merge_class(res)
...
merge_class <- function(res, class = "source") {
  ...
  for (i in rev(seq_len(n - 1))) {
    if (idx[i + 1] - idx[i] == 1) {
      res[[idx[i]]][["src"]] <- c(res[[idx[i]]][["src"]], res[[idx[i + 1]]][["src"]])
      res[[idx[i + 1]]] <- NULL
      idx <- idx[-(i + 1)]
    }
  }
