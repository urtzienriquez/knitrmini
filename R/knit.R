knit = function(
  ...
  if (!child_mode()) {
    ...
    has_user_knitbg = grepl('\\\\definecolor\\{knitbg\\}',
...
compile_pdf = function(tex_file, engine = 'pdflatex', bib_engine = NULL, quiet = FALSE) {
  ...
  run_latex = function(pass) {
    ...
    res = system2(engine, c('-interaction=nonstopmode', '-shell-escape',
                            basename(tex_file)),
  ...
  run_bib = function() {
    if (is.null(bib_engine)) return(invisible(TRUE))
...
detect_bib_engine = function(tex_content) {
  if (grepl('\\\\addbibresource\\{', tex_content)) return('biber')
  if (grepl('\\\\bibliography\\{|\\\\bibliographystyle\\{', tex_content)) return('bibtex')
  NULL
}
...
check_color_definition = function(clean_preamble) {
  if (grepl('\\\\definecolor\\{', clean_preamble) &&
    ...
    stop("You used \\definecolor{...} in your preamble without loading the xcolor package.\n",
         "       Add \\usepackage{xcolor} *before* \\definecolor{...} in your .Rnw file.")
}

parse_latex_log = function(log_path) {
  empty = list(errors = character(), warnings = character(),
  ...
  n = length(content)
  errors = character(); warnings = character(); badboxes = character()
  undefined_refs = character(); undefined_citations = character()
  ...
  list(errors = errors, warnings = warnings, badboxes = badboxes,
